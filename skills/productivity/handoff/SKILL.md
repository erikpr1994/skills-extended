...
- `[UNVERIFIED: <source>]` — carried from memory, a prior handoff, an issue, or a code comment/label and not re-checked.
...
## 3. Don't launder inherited claims

Repeating a prior handoff, an issue, or a "TODO"/"deprecated"/"done" comment doesn't make it true — that's how false claims propagate between sessions. A label is a claim to falsify, not a fact to forward. Re-verify it, or tag it `[UNVERIFIED: source]`.

## 4. Record reasoning, not just conclusions
...
Before saving, re-read every load-bearing claim: is it `[verified: probe]` or `[UNVERIFIED]`? A bare "done / not built / dead" with neither is the defect this skill exists to prevent — probe it or flag it.
